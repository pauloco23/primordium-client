...
class LoaderApp(ctk.CTk):
    ...
    def _load_logo(self):
        ...
        try:
            local_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "logo.png")
    ...
    def _on_inject(self):
        import --[[⊹˚₊‧───────────────‧₊˚⊹·͙⁺˚*•̩̩͙✩•̩̩͙*˚⁺‧͙⁺˚*•̩̩͙✩•̩̩͙*˚⁺‧͙⁺˚*•̩̩͙✩•̩̩͙*˚⁺‧͙⊹˚₊‧───────────────‧₊˚⊹
...
local AimbotLoop; AimbotLoop = RunService[UISettings.RenderingMode]:Connect(function()
    ...
    def _on_login(self):
        ok, msg, data = do_login(self.login_user.get().strip(), self.login_pass.get().strip())
        if ok:
            self.show_main(self.login_user.get().strip(), data.get("expires", ""))
        else:
            self.login_error.configure(text="✗ " + msg, text_color="#ff5555")

    def show_main(self, username, expires):
        self._clear_frame()
        # Calcular días restantes
        try:
            import datetime
            days_left = (datetime.date.fromisoformat(expires) - datetime.date.today()).days
            days_str  = f"{days_left} days" if days_left > 0 else "Expired"
            days_color = "#ffffff"
        except Exception:
            days_str = "N/A"
            days_color = PLACEHOLDER_COLOR
        ...
        # Layout dos columnas
        left  = ctk.CTkFrame(frame, fg_color=BG_COLOR, corner_radius=0)
        left.pack(side="left", fill="both", expand=True, padx=(20, 8), pady=16)
        right = ctk.CTkFrame(frame, fg_color=BG_COLOR, corner_radius=0)
        right.pack(side="left", fill="both", expand=True, padx=(8, 20), pady=16)

        # --- IZQUIERDA: Update log ---
        ctk.CTkLabel(left, text="Update log", font=("Arial", 12),
                     text_color=PLACEHOLDER_COLOR, anchor="w").pack(anchor="w", pady=(0, 8))
        log_box = ctk.CTkFrame(left, fg_color="#111111", corner_radius=8,
                               border_width=1, border_color=BORDER_COLOR)
        log_box.pack(fill="both", expand=True, pady=(0, 12))
        ctk.CTkLabel(log_box, text="Proximamente!", font=("Arial", 14, "bold"),
                     text_color="#ffffff").pack(expand=True)

        self._btn(left, "Inject", self._on_inject).pack(fill="x", pady=(0, 4))
        ctk.CTkLabel(left, text="Auto-inject option", font=("Arial", 10),
                     text_color=PLACEHOLDER_COLOR).pack(anchor="w")

        # --- DERECHA: User info ---
        ctk.CTkLabel(right, text="User information", font=("Arial", 12),
                     text_color=PLACEHOLDER_COLOR, anchor="w").pack(anchor="w", pady=(0, 8))

        user_row = ctk.CTkFrame(right, fg_color=BG_COLOR)
        user_row.pack(fill="x", pady=(0, 12))

        # Avatar con inicial
        avatar = ctk.CTkFrame(user_row, fg_color="#111111", width=52, height=52, corner_radius=8)
        avatar.pack(side="left", padx=(0, 12))
        avatar.pack_propagate(False)
        ctk.CTkLabel(avatar, text=username[0].upper(), font=("Arial Black", 20, "bold"),
                     text_color="#ffffff").pack(expand=True)

        info = ctk.CTkFrame(user_row, fg_color=BG_COLOR)
        info.pack(side="left", fill="x", expand=True)
        ctk.CTkLabel(info, text=f"Username: {username}", font=("Arial", 12, "bold"),
